...
public class Game {
    ...
    public void leave(Player player, boolean teleportToLobby) {
        try {
            ...
            boolean inMatch = MatchManager.hasPlayerMatch(player);
            if (inMatch) {
                if (round != 0) {
                ...
                if (teleportToLobby) {
                    player.teleport(ReviversTwo.getLobbyLocation());
                }
            }
            ...
            ReviversTwo.getPlugin().getLogger().warning("Error removing player from game: " + player.getName());
            e.printStackTrace();
            
            // Ensure player is teleported to lobby even if an error occurs
            if (teleportToLobby) {
                player.teleport(ReviversTwo.getLobbyLocation());
            }
        }
